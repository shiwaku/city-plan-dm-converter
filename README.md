...
| ファイル | 対象 | 内容 |
|---|---|---|
| `都市計画基本図_記号.qml` | 記号（E5） | `Code`（分類コード）でカテゴリ分けし、凡例に公共測量標準図式の名称を表示 |
| `都市計画基本図_方向.qml` | 方向（E6） | `Angle` に従って記号を回転 |
...
本ツールの `Angle` は水平右（東）を0度とする反時計回りですが、**QGIS のマーカー回転は北を0度とする時計回り**です。向きが90度ずれるため、方向のスタイルでは次の式でマーカー角度を与えています。
...
記号のカテゴリは、実データに出現する分類コードから作っています。同梱しているものは静岡市データの68コード分（＋「その他」）です。他自治体のデータで作り直す場合は、変換後に次を実行します。
