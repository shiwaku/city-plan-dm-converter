...
記号（E5）・方向（E6）・注記（E7）は代表点のポイントとして出力するため、GeoJSON や GeoParquet をそのまま QGIS に読み込むと、いずれも同じ点マーカーが並ぶだけになります。地物の種別・向き・文字は属性として持っているので、スタイルを当てて初めて図として読めるようになります。

そのためのレイヤスタイルを [`qgis/`](qgis/) に同梱しています。
...
読み込みは、レイヤプロパティの「スタイル」→「スタイルの読み込み」→「ファイルから」で該当の `.qml` を指定します。データファイルと同じフォルダに同じ名前で置けば（例: `output/都市計画基本図_2500_記号.parquet` に対して `output/都市計画基本図_2500_記号.qml`）、読み込み時に自動で適用されます。
