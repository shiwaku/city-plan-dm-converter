 ...
 */
const PMTILES_BASE =
  import.meta.env.VITE_PMTILES_BASE ?? 'https://shi-works.com/pmtiles/city-shizuoka'
...
/**
 * 補正後の目標 bbox。dm-sprite の設計基準は 10〜22px。
 *
 * 当初は全113アイコンの中央値 18.56px を目標にしたが、それだと補正対象が
 * 基準内のアイコンとまったく同じ大きさになり、広葉樹林などが大きすぎた。
 * 対象の多くは面を埋めるように繰り返し描かれる植生記号（広葉樹林は43,352件）で、
 * 個々の記号として目立つより地模様として読めるほうがよい。基準の下寄りに置く。
 */
const ICON_TARGET_PX = 14
