...
また建物（Code 3001〜3004）は ZL13 未満のタイルから除外しています。`kihonzu_10000_line_building` の `minzoom: 13` と境界が一致するため線レイヤの見た目は変わりませんが、面レイヤは ZL12 以下で建物が描かれなくなります（低ズームでは都市の骨格のみ表示する方針）。詳細は[ルート README の低ズームでの建物の除外](../README.md#低ズームでの建物の除外)を参照してください。
