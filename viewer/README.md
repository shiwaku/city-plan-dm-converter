...
| 属性表示 | 地物クリックでDMの属性をポップアップ表示。分類コードには公共測量標準図式の名称を併記する。タイルには描画に必要な属性（`Code` / `Text` / `Angle`）だけを載せているため、表示されるのはこの範囲になる（[タイルに残す属性](../docs/vector-tiles.md#タイルに残す属性)を参照） |
...
表示している PMTiles は、リポジトリルートの `scripts/build.sh` で生成しています。生成手順・tippecanoe のオプション・最大ズームレベルの決め方は [docs/vector-tiles.md のベクトルタイル作成](../docs/vector-tiles.md#ベクトルタイル作成参考) を参照してください（ここに手順を写すと二重管理になるため置いていません）。
