...
PMTilesファイルは Cloudflare R2（`shi-works.com/pmtiles/city-shizuoka/`）にホスティングしています。タイルの軽量化（1/2,500 が84MB、1/10,000 が39MB）で Git の100MBファイル制限は下回るようになりましたが、更新のたびに履歴が膨らむため引き続き外部ホスティングにしています。

以前はレンタルサーバ（`shiworks2.xsrv.jp`）で配信していましたが、大容量 PMTiles への Range リクエストがオフセット位置に比例して遅くなる特性があり（実測: ファイル先頭 0.06 秒に対し末尾 3 秒超）、描画が重かったため R2 に移しました。R2 ではオフセットに依らずほぼ一定（0.2〜0.5 秒）です。配置は xserver-cleanup のパス規約に従い、第1階層がアクセス方法（`pmtiles/`）、第2階層がデータの出所（`city-shizuoka/`）です。
